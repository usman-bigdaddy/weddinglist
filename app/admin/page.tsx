...
export default function RSVPListPanel() {
  ...
  const fetchRsvps = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/rsvps?filter=${filter}&search=${searchTerm}`);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex space-x-2">
            <button
              ...
              className={`px-4 py-2 rounded-md ${filter === 'all' ? 'bg-pink-600 text-white' : 'bg-pink-100 text-pink-800'}`}
            >
              All
            </button>
            <button
              onClick={() => setFilter('verified')}
              className={`px-4 py-2 rounded-md ${filter === 'verified' ? 'bg-pink-600 text-white' : 'bg-pink-100 text-pink-800'}`}
            >
              Verified
            </button>
            <button
              onClick={() => setFilter('pending')}
              className={`px-4 py-2 rounded-md ${filter === 'pending' ? 'bg-pink-600 text-white' : 'bg-pink-100 text-pink-800'}`}
            >
              Pending
            </button>
          </div>
            ...
            <table className="min-w-full bg-pink-50 rounded-lg overflow-hidden">
              <thead className="bg-pink-100">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-pink-900">RSVP Date</th>
                  <th className="px-4 py-3 text-left text-pink-900">Verified At</th>
                </tr>
              </thead>
              <tbody>
                {filteredRsvps.map((rsvp) => (
                  <tr key={rsvp._id} className="border-b border-pink-200 hover:bg-pink-100">
                    <td className="px-4 py-3 text-pink-900">{rsvp.fullName}</td>
                    <td className="px-4 py-3 text-pink-900">{rsvp.email}</td>
                    <td className="px-4 py-3 text-pink-900 font-mono">{rsvp.otp}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs ${rsvp.isVerified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                    ...
                    <td className="px-4 py-3 text-pink-900 text-sm">
                      {rsvp.verifiedAt ? new Date(rsvp.verifiedAt).toLocaleString() : '--'}
                    </td>
                  </tr>
                ))}
              </tbody>
